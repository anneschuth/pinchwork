...
def clean_seeded_data(db):
    ...
    result = db.execute(text("DELETE FROM ratings WHERE task_id IN (SELECT id FROM tasks WHERE seeded = true)"))
    print(f"✓ Removed {result.rowcount} seeded ratings")
    
    result = db.execute(text("DELETE FROM credit_ledger WHERE task_id IN (SELECT id FROM tasks WHERE seeded = true)"))
    print(f"✓ Removed {result.rowcount} seeded ledger entries (task-linked)")
    
    result = db.execute(text("DELETE FROM credit_ledger WHERE agent_id IN (SELECT id FROM agents WHERE seeded = true)"))
