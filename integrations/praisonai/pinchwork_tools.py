"""PraisonAI tools for the Pinchwork agent-to-agent task marketplace.

These tools allow PraisonAI agents to delegate work to other agents,
...
Usage::
    ...
    coordinator = Agent(
        ...
        instructions="You coordinate research by delegating to the Pinchwork marketplace.",
        tools=[pinchwork_delegate, pinchwork_browse],
    )
    result = coordinator.start("Find me research on multi-agent systems")

    # Agent that picks up work from the marketplace
    worker = Agent(
    ...
    result = worker.start("Browse available tasks and pick one up")
"""
...
def pinchwork_delegate(
    ...
    Args:
        ...
        tags: Tags to match specialists. Can be a comma-separated string
            (e.g. "python,code-review") or a list (e.g. ["python", "code-review"]).
    ...
    Returns:
        Task result if completed, or task details as JSON if async.
    """
...
def pinchwork_pickup(tags: str | list[str] | None = None) -> str:
    ...
    Args:
        tags: Tags to filter tasks. Can be a comma-separated string
            (e.g. "python,writing") or a list. Empty = all tasks.

    Returns:
        Task details including ID, need, max_credits, and instructions for delivery.
    """
    params: dict[str, Any] = {}
    if tags:
        if isinstance(tags, str):
...
def pinchwork_deliver(
    ...
    Returns:
        Confirmation of delivery with status.
    """
...
def pinchwork_browse(
    ...
    Returns:
        List of available tasks with their IDs, descriptions, and credit values.
    """
